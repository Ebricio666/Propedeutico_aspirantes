...
def encontrar_columna(df, posibles_nombres):
    """Encuentra columnas aunque cambien acentos, espacios o mayúsculas."""
...
def normalizar_sexo(valor):
    """Homologa valores de sexo o género."""
...
def clasificar_estado_procedencia(valor):
    """
    Clasifica el estado desde el texto de la escuela.

    Las escuelas que no pueden identificarse se consideran Colima,
    conforme al criterio definido para este proyecto.
    """
...
def normalizar_escuela_procedencia(valor):
    """
    Agrupa las variantes de escuelas similares.

    U de C / Universidad de Colima -> Universidad de Colima (U de C)
    CBTIS / CBTIs / CBTis -> CBTis
    Telebachillerato -> Telebachillerato
    Colegio de Bach / COBACH / COBA -> Colegio de Bachilleres
    """
...
def procesar_hoja(contenido_archivo, nombre_hoja):
    """Lee una hoja y genera registros estandarizados."""
...
def crear_tabla_calificaciones_por_sexo(df):
    """Prepara datos para barras apiladas por sexo."""
...
def crear_resumen_procedencia(df):
    """Crea tabla de procedencia por estado."""
...
def crear_resumen_bachillerato(df):
    """Crea Top 10 de bachilleratos y agrupa los demás como Otros."""
...
def mostrar_grafica_calificaciones(df):
    """Muestra barras apiladas de promedio por sexo."""
...
def mostrar_grafica_procedencia(df):
    """Muestra pastel de procedencia estatal."""
...
def mostrar_grafica_bachillerato(df):
    """Muestra pastel Top 10 de bachilleratos y Otros."""
...
with tab_carrera:
    ...
    with col_bachillerato:
        st.markdown("### Bachillerato de procedencia")
        mostrar_grafica_bachillerato(df_carrera)    texto = unicodedata.normalize("NFD", texto)
